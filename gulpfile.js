...
gulp.task('script', () => {
    ...
    return rollup.rollup({
        ...
        bundle.write({
            ...
            moduleName: 'atEditor',
            // 产出的未压缩的文件名
            dest: './release/atEditor.js'
        }).then(() => {
            // 待 rollup 打包 js 完毕之后，再进行如下的处理：
            gulp.src('./release/atEditor.js')
                // inline css
                .pipe(gulpReplace(/__INLINE_CSS__/gm, function () {
                    // 读取 css 文件内容
                    var filePath = path.resolve(__dirname, 'release', 'atEditor.css')
